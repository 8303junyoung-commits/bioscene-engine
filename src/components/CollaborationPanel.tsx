  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Collaboration hub">
    <section className="phase9-panel">
      ...
      <div className="phase9-body collaboration-grid">
        <div>
          <label data-help="이 장면을 작성·검토하는 사람이나 역할을 쉼표로 구분해 입력하세요. 입력란을 벗어나면 참여자 목록으로 저장되어 Scene과 협업 기록에 포함됩니다.">Participants<input aria-label="Participants" value={participantsDraft} onChange={(event) => setParticipantsDraft(event.target.value)} onBlur={commitParticipants} placeholder="Scientist, reviewer, designer"/></label>
          <div className="comment-compose" data-help="작성자와 과학 검토 의견을 입력하고 Add comment를 누르면 현재 장면의 댓글·활동 기록에 추가됩니다. Resolve는 해결 표시일 뿐 댓글을 삭제하지 않습니다."><input aria-label="Comment author" value={author} onChange={(event) => setAuthor(event.target.value)} placeholder={signedInEmail || 'Author'}/><textarea aria-label="Comment body" rows={3} value={body} onChange={(event) => setBody(event.target.value)} placeholder="Add a scientific review comment"/><button onClick={addComment}><MessageSquare size={14}/> Add comment</button></div>
        ...
        <div className="sync-card">
          ...
          <label data-help="장면 동기화를 처리하는 서버의 기본 주소입니다. 보통 배포된 Supabase Edge Function URL을 그대로 사용하며, 변경하면 서버 리비전 추적이 새로 시작됩니다.">Endpoint<input aria-label="Sync endpoint" type="url" value={room.endpoint} onChange={(event) => onRoomChange({ ...room, endpoint: event.target.value, revision: undefined })} placeholder="https://project.supabase.co/functions/v1/bioscene-api"/></label>
          <label data-help="공동 작업 공간의 고유 이름입니다. 같은 Endpoint와 Room ID를 사용하는 승인된 사용자끼리 같은 장면 리비전을 Push/Pull합니다. 예: il18-study-001.">Room ID<input aria-label="Room ID" value={room.roomId} onChange={(event) => onRoomChange({ ...room, roomId: event.target.value, revision: undefined })} placeholder="study-001"/></label>
          <label data-help="Supabase 로그인 사용 시 Bearer token을 선택합니다. API key는 별도 서버용이며, None은 인증이 없는 로컬 개발 서버에서만 사용하세요.">Authentication<select aria-label="Authentication mode" value={room.authMode} onChange={(event) => onRoomChange({ ...room, authMode: event.target.value as RoomConfig['authMode'] })}><option value="none">None (local development)</option><option value="bearer">Bearer token</option><option value="api-key">API key</option></select></label>
          ...
          <div className="sync-actions"><button data-help="현재 장면 전체를 서버의 다음 리비전으로 업로드합니다. 다른 사용자가 먼저 수정했다면 충돌을 알리고 덮어쓰지 않습니다." disabled={busy || !ready} onClick={onPush}><Send size={14}/> Push revision</button><button data-help="서버의 최신 리비전을 내려받아 현재 장면을 교체합니다. 데이터 손실을 막기 위해 실행 전 확인창과 로컬 스냅샷이 생성됩니다." disabled={busy || !ready} onClick={onPull}><RefreshCw size={14}/> Pull latest</button></div>
