    ...
    <aside className="inspector">
      ...
      {selectedNode?.data.kind === 'membrane' && selectedNode.data.membrane && (() => {
        ...
        return <div className="inspector-form membrane-inspector">
          ...
          <label data-help="Simple은 굵은 띠, Standard는 성긴 인지질 이중층, Detailed는 headgroup과 두 개의 hydrophobic tail을 촘촘히 표시합니다. 생물학적 앵커 데이터는 동일하게 유지됩니다.">Visual style<select value={membrane.style} onChange={(event)=>changeMembrane({style:event.target.value as MembraneDefinition['style']})}><option value="simple">Simple band</option><option value="standard">Standard bilayer</option><option value="detailed">Detailed lipid bilayer</option></select></label>
